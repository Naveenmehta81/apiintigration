...
const Product = () => {
  ...
  const { data, isLoading, isError } = useQuery({
    queryKey: ["products", search, currentPage], // The label for this data
    queryFn: async () => {
      ...
      const response = await fetch(
        `${baseUrl}?limit=${limitData}&skip=${skip}`,
      );
  ...
  const totalPages = data?.total ? Math.ceil(data.total / limitData) : 0;
  ...
  return (
    <div className="select-none cursor-default min-h-screen bg-gray-50 pb-10">
      {/* 1. Header Section */}
            ...
            <div className="grid gap-8 grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
              {products.map((p) => (
                ...
                >
                  ...
                  <div className="p-5">
                    <h2
                      className="font-bold text-gray-800 text-lg mb-2 truncate"
                      title={p.title}
                    >
                      {p.title}
                    </h2>
